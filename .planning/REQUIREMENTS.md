...
- [x] **HUB-05** (DERIVED): Captures, chat-decision promotions, and chat-session closes write to `{wikiPath}/raw/aios/<kind>-YYYY-MM-DD-<slug>.md` (kinds: `capture`, `chat-decision`, `chat-session`); files are immutable once written; AIOS never writes to curated wiki paths — *Capture branch delivered 2026-05-21 by Plan 04-05; chat-decision + chat-session branches land in 04-08*
- [x] **HUB-06** (DERIVED): Every write emits a Receipt feed entry (timestamp, kind, project, absolute file path, excerpt) visible to the operator on Home and on the relevant Project page — *Capture branch delivered 2026-05-21 by Plan 04-05 (subprocess fallback suppresses receipt when no parseable path); remaining write surfaces emit via 04-08/04-09*
...
| HUB-05 | Phase 4 (v2 Bidirectional Hub) | Capture branch complete (04-05); chat-decision + chat-session branches pending (04-08) |
| HUB-06 | Phase 4 (v2 Bidirectional Hub) | Capture branch complete (04-05); chat + ingest receipts pending (04-08/04-09) |
