...
The LLM wiki pattern is already implemented in Justin's AIOS (the `/wiki` skill). Nate's implementation validates the approach and adds specific operational practices: the hot cache for token savings, the daily/weekly loops for continuous improvement, and the skill-as-SOP framing. When evaluating knowledge management approaches for client projects, the wiki-over-RAG position is the default recommendation. The feedback loop discipline (nothing breaks twice) applies to every automation BGD builds.

**Update 2026-06-17:** Nate expanded this into a five-level taxonomy in `second-brain-levels-framework.md`. The LLM wiki is Level 2 in that framework. His position is nuanced: wiki-over-RAG is the default, but semantic search (Level 3) and knowledge graphs (Level 4) have legitimate use cases for specific data types within the same project. The key insight is mixed-level architecture — different folders at different levels based on pain, not aspiration.
